...
function displayBanner() {
  ...
  const banner = figlet.textSync('ADB NODE', { font: 'ANSI Shadow' });
  banner.split('\n').forEach(line => {
    console.log(chalk.cyanBright(line.padStart(line.length + Math.floor((width - line.length) / 2))));
  });
  console.log(chalk.cyanBright(' '.repeat((width - 50) / 2) + '=== Telegram Channel 🚀 : ADB NODE ( @airdropbombnode ) ==='));
...
async function processAccounts(privateKeys, messages, accountProxies, chatCount, noType) {
  ...
  for (let i = 0; i < privateKeys.length; i++) {
    ...
    try {
      ...
      console.log(chalk.red(`✗ Invalid private key for account ${i + 1}`));
      failCount++;
      continue;
    }
    ...
    console.log(chalk.blue(`═════[ Account ${i + 1}/${privateKeys.length} | ${shortAddress} @ ${getTimestamp()} ]═════`));
